// frontend/src/pages/schedule/DutyBoardTab.tsx
...
// Staff row in assignment dialog
function StaffRow({
  employee,
  onSelect,
}: {
...
function AssignmentDialogContent({
  ...
  const { data: available, isLoading } = useAvailableStaff(date, blockIndex, positionId, true)
      ...
      <div>
        <h4 className="text-sm font-medium mb-2">
          ...
          <span className="text-xs text-muted-foreground ml-2">
            (sorted by fewest hours at {positionName})
          </span>
        </h4>
        {isLoading ? (
          ...
          <div className="border rounded-lg divide-y max-h-[300px] overflow-y-auto">
            {available.map((emp) => (
              <StaffRow key={emp.user_id} employee={emp} onSelect={() => onAssign(emp.user_id)} />
            ))}
          </div>
        )}
      </div>
...
function ConsoleHoursSheet({ dateStr }: { dateStr: string }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Console hours for {format(startDate, 'MMMM yyyy')} (through {format(displayDate, 'MMM d')})
      </p>
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground text-center py-8">
          No duty assignments this month
        </p>
      ) : (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b">
                <th scope="col" className="text-left py-1.5 px-2 font-medium">
                  Name
                </th>
                {positions.map((p) => (
                  <th scope="col" key={p} className="text-right py-1.5 px-2 font-medium">
                    {p}
                  </th>
                ))}
...
export default function DutyBoardTab({ date }: DutyBoardTabProps) {
  const [activeCell, setActiveCell] = useState<{ positionId: string; blockIndex: number } | null>(
    null
  )
  ...
  // No refetchInterval — this is the interactive tab, not the kiosk display
  const { data: board, isLoading, isError, refetch } = useDutyBoard(date)
  const cellAction = useCellAction(date)
  const createDatePosition = useCreateDatePosition(date)
  const deleteDatePosition = useDeleteDatePosition(date)
  ...
  const handleCellClick = useCallback(
    (positionId: string, blockIndex: number) => {
      if (!isManager) return
  ...
  const activePosition = activeCell
    ? board?.positions.find((p) => p.id === activeCell.positionId)
    : null
  const activeAssignment = activeCell
    ? assignmentMap.get(`${activeCell.positionId}:${activeCell.blockIndex}`)
    : undefined
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Sheet>
          ...
          <SheetContent className="sm:max-w-xl overflow-y-auto">
            ...
            <ConsoleHoursSheet dateStr={date} />
          </SheetContent>
        </Sheet>
      </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-muted/40">
                <th
                  scope="col"
                  className="border px-3 py-2 text-left font-semibold whitespace-nowrap w-[100px]"
                >
                  Console
                </th>
              ...
              {board.positions.map((position) => {
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      'hover:bg-muted/20',
                      position.board_date && 'bg-blue-50/50 dark:bg-blue-950/20'
                    )}
                  >
                    {/* Position label cell */}
                    <td
                      className="border px-3 py-1 font-semibold text-xs whitespace-nowrap"
                    ...
                    {Array.from({ length: 12 }, (_, blockIndex) => {
                        ...
                        return (
                          <td
                            ...
                            className={cn(
                              ...
                              blockIndex === currentBlock && 'ring-1 ring-inset ring-blue-400/40',
                              isManager &&
                                'cursor-pointer hover:bg-yellow-200 dark:hover:bg-yellow-900/60',
                            ...
                            {...(isManager
                              ? {
                                  ...
                                  'aria-label': `${position.name} ${BLOCK_LABELS[blockIndex]}: OT needed`,
                                }
                              : {})}
                          >
                            OT
                      ...
                      if (assignment?.status === 'assigned' && assignment.user_first_name) {
                        const isOwnCell = assignment.user_id === currentUserId
                        return (
                          <td
                            key={blockIndex}
                            className={cn(
                              'border px-1 py-0.5 text-center text-xs truncate max-w-[80px]',
                              isOwnCell ? 'font-bold' : 'font-medium',
                              blockIndex === currentBlock && 'ring-1 ring-inset ring-blue-400/40',
                            ...
                            {...(isManager
                              ? {
                                  ...
                                  role: 'button' as const,
                                  onKeyDown: cellKeyDown,
                                  'aria-label': `${position.name} ${BLOCK_LABELS[blockIndex]}: ${assignment.user_first_name} ${assignment.user_last_name}`,
                                }
                              : {})}
                          >
                        ...
                        <td
                          ...
                          {...(isManager
                            ? {
                                ...
                                role: 'button' as const,
                                onKeyDown: cellKeyDown,
                                'aria-label': `${position.name} ${BLOCK_LABELS[blockIndex]}: empty`,
                              }
                            : {})}
                        >
                          &nbsp;
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {activePosition?.name} &mdash;{' '}
              {activeCell ? BLOCK_LABELS[activeCell.blockIndex] : ''}-
              {activeCell ? BLOCK_LABELS[activeCell.blockIndex + 1] ?? '2400' : ''}
            </DialogTitle>
            <DialogDescription>
              Assign staff or mark overtime for this time block.
            </DialogDescription>
          </DialogHeader>
      ...
      {/* Add date-specific position dialog */}
      <Dialog open={addRowOpen} onOpenChange={setAddRowOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              Add Position for {format(new Date(date + 'T12:00:00'), 'MMM d')}
            </DialogTitle>
          ...
          >
            <div className="space-y-2">
              <Label htmlFor="duty-tab-row-name">Position Name</Label>
              <Input
                id="duty-tab-row-name"
            ...
            <div className="space-y-2">
              <Label htmlFor="duty-tab-row-class">Classification (optional)</Label>
              <Select value={newRowClassification} onValueChange={setNewRowClassification}>
                <SelectTrigger id="duty-tab-row-class">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                type="submit"
                disabled={!newRowName.trim() || createDatePosition.isPending}
              >
                Add Position
              </Button>
            </div>
